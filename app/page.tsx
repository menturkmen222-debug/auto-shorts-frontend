...
export default function DashboardPage() {
  ...
  const uploadFile = async (id: string, file: File, prompt: string, channelName: string) => {
    ...
    try {
      const response = await fetch('https://https://autooz.tkmjoker89.workers.dev/upload-video', {
        method: 'POST',
        body: formData,
      });
      ...
      setTimeout(async () => {
        try {
          const retryRes = await fetch('https://https://autooz.tkmjoker89.workers.dev/upload-video', {
            method: 'POST',
            body: formData,
          });
          if (retryRes.ok) {
  ...
  const uploadByUrl = async (id: string, url: string, prompt: string, channelName: string) => {
    ...
    try {
      const res = await fetch('https://https://autooz.tkmjoker89.workers.dev/upload-video', {
        method: 'POST',
        body: formData,
      });
